...
# 3. 在侧边栏创建登录框
# 返回三个状态：昵称, 登录状态(True/False/None), 用户名
...
if authentication_status is False:
    ...
    # 显示一个漂亮的未登录封面页 (替代原来的功能页)
    st.title("🔒 欢迎来到 VocabMaster Pro")
    st.markdown("### 您的智能生词本专家")
    st.info("为了保护数据安全并提供个性化服务，请先在左侧侧边栏登录。")
    st.markdown("---")
    c1, c2, c3 = st.columns(3)
    c1.metric("⚡ 智能提取", "支持 SRT/DOCX/TXT")
    c2.metric("🛡️ 熟词屏蔽", "内置多级词库")
    c3.metric("☁️ 云端同步", "对接 GitHub")
...
# 登录成功！在侧边栏顶部显示欢迎语和登出按钮
    ...
    with st.sidebar:
        ...
        st.caption("v12.0 Pro Login Edition") # 更新版本号
        st.markdown("---")

这是一个极具挑战性但也非常有趣的重构任务。你的目标是将原本基于 Python/Streamlit 的功能逻辑，嵌入到那套基于 HTML/Vue/Tailwind 设计的高级 UI 皮肤中。

由于 Streamlit 的底层渲染机制限制，我们不能直接“复制粘贴” HTML/Vue 代码。**但是，我们可以通过大量的自定义 CSS 和布局技巧，在 Streamlit 中 1:1 复刻那套 UI 的视觉风格。**

以下是完全重构后的 `app.py`。我保留了所有的核心逻辑（NLTK/Spacy/Github），但将界面完全重写为 **"Teal/Slate" (青色/岩石灰)** 风格，使用了类似 Tailwind 的设计系统。

### 主要 UI 升级点：

1.  **配色系统**：从原本的 Indigo 换成了 HTML 模板中的 **Teal (\#0F766E / \#00B99B)** 和 **Slate (\#F8FAFC)** 配色。
2.  **布局复刻**：
      * **侧边栏**：去除了原生丑陋的边框，改为纯净的白色面板 + 悬浮效果菜单。
      * **工作台 (Workbench)**：采用了 **左侧配置卡片 (1/3) + 右侧操作区 (2/3)** 的非对称布局，还原 HTML 设计。
      * **书库 (Library)**：通过 CSS Grid 和 HTML 注入，完美复刻了 **3D 悬停图书封面** 效果。
3.  **组件美化**：
      * 输入框去除默认边框，改为柔和的背景色。
      * 按钮增加了 Hover 动画和阴影。
      * 文件上传区改为虚线拖拽风格。
